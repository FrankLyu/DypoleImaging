    ...
    try:
        ...
        for i in range(NUM_IMAGES):
            try:
                ...
                image_result = cam.GetNextImage(100000) # time in second it will wait for the buffer to fill (so basically when
...
# Run example on each camera
for i, lescam in enumerate(cam_list):
    cam = lescam
...
cam.AcquisitionMode.GetAccessMode()
cam.AcquisitionMode.SetValue(PySpin.AcquisitionMode_Continuous)
print('Acquisition mode set to continuous...')
...
for i in range(10):
    ...
    grab_next_image_by_trigger(cam)
    try:
        image_result = cam.GetNextImage(1000) # time in second it will wait for the buffer to fill (so basically when
                                                # at the latest can the trigger pulse arrive
