...
def fermionDistribution(xy, x0, y0, sigma, A, zeta, offset):
    """FermionParams = ?"""
    x,y=xy
    denom = fermi_poly2( np.log(zeta) - (x - x0) **2/(2*sigma**2) - (y - y0)**2/(2*sigma**2) )
...
def FermionFit2D(data, Initialparams):
    ...
    Z = data  # Your 2D data
    # Initial guess for the parameters
    initial_guess = (Guessx_center, Guessy_center, np.sqrt(Guessx_width*Guessy_width),1, 5, 0)
    lower_bounds = [-np.inf, -np.inf, 0, 0.2, 0, -0.2]  # Lower bounds for each parameter
    upper_bounds = [np.inf, np.inf, np.inf, np.inf, np.inf, 0.2]  # Upper bounds for each parameter
    # Define specific bounds for each parameter as needed
    param_bounds = (lower_bounds, upper_bounds)
    # Perform the fit
    if data.shape[0]*data.shape[1]>400*400:
        xc, yc, sigma, zeta, modifiedrawAtomNumber, isFitSuccessful= 0,0,0,0,0,0
        ...
        try:
            ...
            print(params)
            # Unpack the parameters
            xc, yc, sigma, A, zeta, offset = params #TF refers to Thomas Fermi
            isFitSuccessful = True
            Zfit = fermionDistribution((X, Y), xc, yc, sigma, A, zeta, offset).reshape(X.shape)
            modifiedrawAtomNumber = np.sum(data)-offset*np.size(data)
            Fermionfit_xsum = np.sum(Zfit, axis = 0)
            Fermionfit_ysum = np.sum(Zfit, axis = 1)
            
        except Exception:
            xc, yc, sigma, A, zeta, offset = 0, 0, 0, 0, 0, 0
            modifiedrawAtomNumber = 0
            isFitSuccessful = False
            Zfit = np.ones_like(Z)
            Fermionfit_xsum = np.zeros_like(x)
    ...
    return xc, yc, sigma, zeta, modifiedrawAtomNumber, isFitSuccessful, Fermionfit_xsum, Fermionfit_ysum, Zfit
