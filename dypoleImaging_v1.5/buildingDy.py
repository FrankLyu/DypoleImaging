...
######################Read the image data when new files go into andor/images and do transform###########################
    ...
    def setAtomImageAsDivision(self, defringing = False):
        ...
        else: # Contrary to prior version, JL chooses not to correct NoAtom. This only has to do with the canvas show. Different from final atom count.
...
#Show Img on the main canvas axe1, another main function of this part is AOI selection, SEPARATE coded in _AOIstuff.py#
 ...
 #######After AOI selected, count atom number and do fitting for your AOI, show 1D fit on axe2,3############
    ...
    def Fit1D(self, axis = 'xy'):
        try:
            self.doGaussianFit(axis)
            activeAOI = self.primaryAOI
            #For detailed description of this part, refer to older version. Now we only do Gaussian Fit.
    ...
    def update1DProfiles(self):
        ...
        for activeAOI in self.AOIList:
            ...
            try:                           
                ...
                xMax = 2
                xMin = 1
            #JL temperory change on 10/18/2023
            ...
            #JL temperory change on 10/18/23
...
#            print "----------Let's kill the old observer---------------------"
    ...
    def dictionnaryAnalysisResults(self):
        analysisResults = {
            #"nCount" : int(self.atomNumber),  ##JL modified on 10/25, now we save 2D fit number and width
            "nCount" : int(self.primaryAOI.modifiedAtomNumber),
            "xWidth" : self.primaryAOI.x_width2D,
            "yWidth" : self.primaryAOI.y_width2D,
            "xPos" : self.primaryAOI.x_center,
            "yPos" : self.primaryAOI.y_center,
            "PSD" : 0
            }
            # JL modified on 11/17 to see difference
        if self.doubleAOIs:
...
if __name__ == '__main__':
    app = wx.App()
    ui = ImageUI(None, title='Atom Image Analysis Dy v1.5')
    app.MainLoop()