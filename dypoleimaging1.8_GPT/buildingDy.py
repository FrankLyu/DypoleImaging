...
class ImageUI(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.cropYStart = 1
        self.cropYEnd = 2160
        self.cameracrop = 250 #JH redo the crop thing
        ...
        self.imageAngle = 37.3
        self.imagePivotX = 970
        self.imagePivotY = 1200
    ...
    from _AOIstuff import settempAOIxc,settempAOIyc,settempAOIw,settempAOIh, settempAOIa, isAOI_PrimaryOutside, isAOI_SecondaryOutside, on_press, on_motion, on_release, typedAOI
...
######################Initialize#############################
    def initializeDummyData(self):
        ...
        #Picture with atoms
        dummyPWA = np.exp(-np.exp(-(X-size/2)**2/(2*sigmax**2) - (Y-size/2)**2/(2*sigmay**2)))
        #Try fermion#
        zeta = 100
        offset = 0.0
        denom = fermi_poly2( np.log(zeta) - (X - size*0.5) **2/(2*sigmax**2) - (Y - size*0.5)**2/(2*sigmay**2) )
        numer = fermi_poly2( np.log(zeta) )
        thermalPart = denom / numer  #Mingwu Lu thesis about first Dy dFg
        dist = 0.3*thermalPart + offset
        dummyPWA = np.exp(-dist)
        ###########
        dummyPWOA =  np.ones_like(X) - generateNoise(np.shape(X))
    ...
    def initializeAOI(self):
        if (self.isAOI_PrimaryOutside() and self.isAOI_SecondaryOutside()):
            ...
            self.AOIxc_Primary.SetValue(str(self.primaryAOI.position[0]))
            self.AOIyc_Primary.SetValue(str(self.primaryAOI.position[1]))
            self.AOIw_Primary.SetValue(str(self.primaryAOI.position[2]-self.primaryAOI.position[0]))
            self.AOIh_Primary.SetValue(str(self.primaryAOI.position[3]-self.primaryAOI.position[1]))
            self.AOIa_Primary.SetValue(str(self.primaryAOI.angle))
...
##################Other settings might be changed within _InitUIstuff.py########################
    ...
    def dorotation(self):
        self.isRotationNeeded = self.checkrotation.GetValue()
        try:
            if self.isRotationNeeded is True:
            	rotangle = self.imageAngle
            else:
            	rotangle = -self.imageAngle #Rotate back
            temp = time.time()
            for i in range(len(self.imageData)):
                self.imageData[i] = self.rotateImg(self.imageData[i], rotangle, [self.imagePivotX, self.imagePivotY])
            print("Time taken Rotation " + str(time.time()-temp))
        except Exception:
            raise Exception("========= rotation ERROR =======")
        self.setAtomImageAsDivision()
        self.setTransformedData() 
        self.showImg()
    ...
    def setCameraPosition(self, e):
        ...
        self.magnif.SetValue(str(self.magnification))
        self.setConstants()
        self.setAtomNumber()
        self.camera = Camera(self.cameraType, self.cameraPosition)
        print(self.camera)
...
######################Read the image data when new files go into andor/images and do transform###########################
    ...
    def setRawDataFromCamera(self, imagePathList):   # called when received images from camera
        ...
        try:
            if self.isRotationNeeded is True:
                for i in range(len(self.imageData)):
                        self.imageData[i] = self.rotateImg(self.imageData[i], self.imageAngle, [self.imagePivotX, self.imagePivotY])
        ...
        print("Here image data after readout. (JL deleted the printout)")
    ...
    def setTransformedData(self, pca = False, gaussianFilter = False, histogramEqualization = False):
        '''
    ...
    def rotateImg(self, img, angle, pivot):
        (h,w) = img.shape[:2]
...
#Show Img on the main canvas axe1, another main function of this part is AOI selection, SEPARATE coded in _AOIstuff.py#
 ...
 #######After AOI selected, count atom number and do fitting for your AOI, show 1D fit on axe2,3############
    ...
    def setAtomNumber(self):
        ...
        self.bigNcount2.SetValue(str('%1.2e'%(self.primaryAOI.rawAtomNumber)))
    ...
    def Fit2DGaussian(self):
        for AAOI in self.AOIList:
            ...
            AAOI.xc2D, AAOI.yc2D, AAOI.x_width2D, AAOI.y_width2D, AAOI.modifiedrawAtomNumber, AAOI.isFit2DSuc, AAOI.ODpeak=gaussianFit2D(AAOI.ODImg,Initialparams)
    ...
    def Fit2DFermion(self):
        for AAOI in self.AOIList:
            Initialparams = [AAOI.x_width,AAOI.y_width,AAOI.x_center,AAOI.y_center,AAOI.position[0],AAOI.position[1],AAOI.isXFitSuccessful,AAOI.isYFitSuccessful]
            AAOI.xc2D, AAOI.yc2D, AAOI.x_width2D, AAOI.y_width2D, AAOI.fermionzeta, AAOI.modifiedrawAtomNumber, AAOI.isFit2DSuc,AAOI.x_fitted,AAOI.y_fitted,self.Zfit=FermionFit2D(AAOI.ODImg,Initialparams)
            AAOI.modifiedAtomNumber = AAOI.modifiedrawAtomNumber * (self.pixelToDistance**2)/self.crossSection
    ...
    def doGaussianFit(self, axis = 'xy'):
        if axis == 'xy': # Regular fit on 2 axis
            for activeAOI in self.AOIList:
                activeAOI.x_center, activeAOI.x_width, activeAOI.x_offset, activeAOI.x_peakHeight, activeAOI.x_fitted, activeAOI.isXFitSuccessful, activeAOI.x_slope, err_x = gaussianFit(activeAOI.x_basis, activeAOI.x_summed, activeAOI, self.isWingsfit,axis = 'x' )
                ...
                activeAOI.y_center, activeAOI.y_width, activeAOI.y_offset, activeAOI.y_peakHeight, activeAOI.y_fitted, activeAOI.isYFitSuccessful, activeAOI.y_slope , err_y= gaussianFit(activeAOI.y_basis, activeAOI.y_summed, activeAOI, self.isWingsfit,axis = 'y' )
            ...
            self.x_center, self.x_width, self.x_offset, self.x_peakHeight, self.x_fitted, self.isXFitSuccessful, self.x_slope, err_x = gaussianFit(self.x_basis, self.x_summed, self.AOI_Primary, self.isWingsfit, axis = 'x' )
            ...
            self.y_center, self.y_width, self.y_offset, self.y_peakHeight, self.y_fitted, self.isYFitSuccessful, self.y_slope, err_y = gaussianFit(self.y_basis, self.y_summed, self.AOI_Primary, self.isWingsfit, axis = 'y')
...
#            print "----------Let's kill the old observer---------------------"
        ...
        if self.buttonSaveDummy.GetValue(): # if you save a dummy image
            ...
            try:
                ...
                writeImageToCacheC(atomShotCropList,
                               ...
                               darkShotCropList,
                               self.cameracrop ,
                               self.cameracrop ,   # self.cameracrop is the width and height of camera ID =7. Check it in mysql.
                               7,
                               lastRunID,
                               lastSequenceID)
            except:
        ...
        print("autorun done 12/4/2024 JL.")
    ...
    def dictionnaryAnalysisResults(self):
        ...
        if self.doubleAOIs:
            analysisResults["nCount2"] = int(self.primaryAOI.modifiedAtomNumber)
            analysisResults["xWidth2"] = self.primaryAOI.x_width2D
            analysisResults["yWidth2"] = self.primaryAOI.y_width2D
            analysisResults["xPos2"] = self.primaryAOI.xc2D
            analysisResults["yPos2"] = self.primaryAOI.yc2D
        ...
        if self.fitMethodBoson.GetValue():
            analysisResults["xWidth"]=self.primaryAOI.tfx_width2D
            analysisResults["yWidth"]=self.primaryAOI.tfy_width2D
        if self.fitMethodFermion.GetValue():
            ToverTF=(fermi_poly3(np.log(self.primaryAOI.fermionzeta))*6)**(-1/3)
            analysisResults["PSD"]= ToverTF[0]
            # print("T/T_F = " + str((fermi_poly3(np.log(self.primaryAOI.fermionzeta))*6)**(-1/3)))
...
#######################################
    ...
    def convertCropImagesToList(self):
        XS = self.primaryAOI.position[1]
        YS = self.primaryAOI.position[0]
        atom = self.imageData[0].astype(np.int16)[XS:XS+self.cameracrop, YS:YS+self.cameracrop].ravel().tolist()
        noatom = self.imageData[1].astype(np.int16)[XS:XS+self.cameracrop, YS:YS+self.cameracrop].ravel().tolist()
        dark = self.imageData[2][XS:XS+self.cameracrop, YS:YS+self.cameracrop].astype(np.int16).ravel().tolist()
        return atom, noatom, dark
...
if __name__ == '__main__':
    app = wx.App()
    ui = ImageUI(None, title='Atom Image Analysis Dy v1.8')
    app.MainLoop()