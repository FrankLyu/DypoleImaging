...
from _helpstuff import FermiTempCalculatorFrame, KaDiCalculatorFrame
...
def InitUI(self):
    ...
    KD_item = helpMenu.Append(wx.ID_ANY, "&Kapitza Dirac", "KD")

    # Add menus to the menu bar
    menuBar.Append(helpMenu, "&Help Function")
    self.SetMenuBar(menuBar)

    self.Bind(wx.EVT_MENU, self.KDcalc, KD_item)

    
    #WXSUPPRESS_SIZER_FLAGS_CHECK = True   
    ...
    self.checkwings = wx.CheckBox(self.panel, label="Wings fit")
    self.Bind(wx.EVT_CHECKBOX, self.dowingsfit, id = self.checkwings.GetId())
    ...
    settingBoxSizer.Add(self.checkwings, flag = wx.ALL | wx.EXPAND, border = 5)
...
#        fittingBox.SetMaxSize((250, 400))
    ...
    self.Fit2Dshowbutton = wx.Button(self.panel, label="Fit2Dshow")
    self.Fit2Dshowbutton.Bind(wx.EVT_BUTTON, self.Fit2Dshow)
    self.Tfcalcbutton = wx.Button(self.panel, label="Fermi Temperature Calculator")
    self.Tfcalcbutton.Bind(wx.EVT_BUTTON, self.FermiTempcalc)
    ...
    fittingBoxSizer.Add(self.Fit2Dshowbutton, flag=wx.ALL|wx.EXPAND, border= 5)
    fittingBoxSizer.Add(self.Tfcalcbutton, flag=wx.ALL|wx.EXPAND, border= 5)
...
#        self.setDefringingRefPath()
    ...
    vbox0.Add(fileBoxSizer,flag=wx.ALL| wx.EXPAND, border = 5)
...
# 4 layer radio buttons
    ...
    self.cursorZ = wx.TextCtrl(self.panel, style=wx.TE_READONLY|wx.TE_CENTRE,  size = (65, 22))
    ...
    #########
    ## defringing option ##
    #hbox43 = wx.BoxSizer(wx.HORIZONTAL) useless?
...
#   self.checkSaveAsRef = wx.CheckBox(self.panel, label = "Save as ref")
    ...
    rotationBoxSizer = wx.StaticBoxSizer(rotationBox, wx.VERTICAL)
    self.checkrotation = wx.CheckBox(self.panel, label = "Rotate?")
    self.Bind(wx.EVT_CHECKBOX, lambda e: self.dorotation(), id = self.checkrotation.GetId())
    rotationBoxSizer.Add(self.checkrotation, flag = wx.ALL, border = 5)
    rotationstr='PivotX='+str(self.imagePivotX)+', PivotY='+str(self.imagePivotY)+', a='+str(self.imageAngle)
    rotationText = wx.StaticText(self.panel, label = rotationstr)
    rotationBoxSizer.Add(rotationText, flag=wx.ALL, border=5)

    ## define color bar vmax ##
    vmax_def = wx.StaticText(self.panel, label = 'Vmax:')
    self.vmax_def = wx.TextCtrl(self.panel, value= str(self.vmax), size=(30,22))
...
#        self.bigNcount = wx.TextCtrl(self.panel,  style=wx.TE_READONLY)
    ...
    aoi_PrimaryText = wx.StaticText(self.panel, label = 'Primary AOI: xc,yc,w,h,a')

    hbox14_Primary.Add(aoi_PrimaryText, flag=wx.ALL, border=5)
    self.AOIxc_Primary = wx.TextCtrl(self.panel, value='-1', size=(40,22))
    self.AOIyc_Primary = wx.TextCtrl(self.panel, value='-1', size=(40,22))
    self.AOIw_Primary = wx.TextCtrl(self.panel, value='-1', size=(40,22))
    self.AOIh_Primary = wx.TextCtrl(self.panel, value='-1', size=(40,22))
    self.AOIa_Primary = wx.TextCtrl(self.panel, value='-1', size=(40,22))
    hbox14_Primary.Add(self.AOIxc_Primary, flag=wx.ALL, border=2)
    hbox14_Primary.Add(self.AOIyc_Primary, flag=wx.ALL, border=2)
    hbox14_Primary.Add(self.AOIw_Primary, flag=wx.ALL, border=2)
    hbox14_Primary.Add(self.AOIh_Primary, flag=wx.ALL, border=2)
    hbox14_Primary.Add(self.AOIa_Primary, flag=wx.ALL, border=2)
    self.AOIxc_Primary.Bind(wx.EVT_TEXT, self.settempAOIxc)
    self.AOIyc_Primary.Bind(wx.EVT_TEXT, self.settempAOIyc)
    self.AOIw_Primary.Bind(wx.EVT_TEXT, self.settempAOIw)
    self.AOIh_Primary.Bind(wx.EVT_TEXT, self.settempAOIh)
    self.AOIa_Primary.Bind(wx.EVT_TEXT, self.settempAOIa)
...
#        tempSizer = wx.StaticBoxSizer(tempStaticBox, wx.HORIZONTAL)
    ...
    imageZoomBoxSizer.Add(hboxZoomInput, 0)
    ...
    self.cursorZ_Zoom = wx.TextCtrl(self.panel, style=wx.TE_READONLY|wx.TE_CENTRE,  size = (65, 22))
    ...
    imageZoomBoxSizer.Add(hboxZoomOutput, 0)
    vbox2.Add(imageZoomBoxSizer, 0, border = 5)
...
def updateFittingResult(self):
    ...
    if self.fitMethodBoson.GetValue() is True:
        for AAOI in self.AOIList:
            ...
            self.resultstring += f"2D fit thermal width: {AAOI.x_width2D:.1f} pixels\n" 
            self.resultstring += f"2D fit BEC width: {AAOI.tfx_width2D:.1f} pixels, {AAOI.tfy_width2D:.1f} pixels\n" 
            self.resultstring += f"2D fit BEC fraction: {AAOI.BECfraction*100:.1f} % \n\n"

    elif self.fitMethodGaussian.GetValue() is True:
        for AAOI in self.AOIList:
            if AAOI==self.primaryAOI:
                self.resultstring += "AOI1 Fitted parameters: \n"
            else:
        ...
        for AAOI in self.AOIList:
            ToverTF=(fermi_poly3(np.log(AAOI.fermionzeta))*6)**(-1/3)
            if AAOI==self.primaryAOI:
                self.resultstring += "AOI1 Fitted parameters: \n"
            else:
                self.resultstring += "AOI2 Fitted parameters: \n"
            ...
            self.resultstring += f"1D fit x_width: {AAOI.x_width:.1f} pixels"
            self.resultstring += f". y_width: {AAOI.y_width:.1f} pixels\n"
            self.resultstring += f"2D fit  x_width: {AAOI.x_width2D:.1f} pixels\n" 
            self.resultstring += f"2D fit  y_width: {AAOI.y_width2D:.1f} pixels\n" 
            self.resultstring += f"Fermion Zeta: {AAOI.fermionzeta:.1f}  \n"
            self.resultstring += f"Obtained from Zeta, T/TF={ToverTF[0]:.3f} \n\n"
...
def Fit2Dshow(self, event):
    Fit2Dshow_frame = Fit2Dshowframe(None, "Fit2Dshow",[self.Zfit,self.AOIList[0].ODImg])
    Fit2Dshow_frame.Show()

class Fit2Dshowframe(wx.Frame):
    def __init__(self, parent, title, data):
        super(Fit2Dshowframe, self).__init__(parent, title=title, size=(600, 600))
        self.figure, (self.ax1, self.ax2) = plt.subplots(1, 2, figsize=(10, 5))
        self.canvas = FigureCanvas(self, -1, self.figure)
        data1=data[0]
        data2=data[1]-data[0]
        # Plot data on the first subplot
        cax1 = self.ax1.imshow(data1)
        self.figure.colorbar(cax1, ax=self.ax1)  # Add colorbar
        self.ax1.set_title("Plot 1 Title")

        # Plot data on the second subplot
        cax2 = self.ax2.imshow(data2,vmin=-0.5,vmax=+0.5)
        self.figure.colorbar(cax2, ax=self.ax2)  # Add colorbar
        self.ax2.set_title("Plot 2 Title")
...
def FermiTempcalc(self, event):
    Tf_frame = FermiTempCalculatorFrame(None, "FermTemp Calculator")
    Tf_frame.Show()

def KDcalc(self, event):
    KD_frame = KaDiCalculatorFrame(None, "KD Calculator")
    KD_frame.Show()