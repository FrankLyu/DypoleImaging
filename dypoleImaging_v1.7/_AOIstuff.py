...
def settempAOIxc(self,e):
    try:
        value = int(e.GetEventObject().GetValue())
        if value > 0:
            self.tempAOIxc = value
    except:
        pass

def settempAOIyc(self,e):
    try:
        value = int(e.GetEventObject().GetValue())
        if value > 0:
            self.tempAOIyc = value
    except:
        pass

def settempAOIw(self,e):
    try:
        value = int(e.GetEventObject().GetValue())
        if value > 0:
            self.tempAOIw = value
...
def on_release(self, event):
    ...
    if event.button == 1:
        ...
        self.AOIa_Primary.SetValue(str(0))
...
def typedAOI(self, event):
    ...
    print(self.tempAOIa)
    activeAOI.position[0]=self.tempAOIxc
    activeAOI.position[1]=self.tempAOIyc
    activeAOI.width=self.tempAOIw
    activeAOI.height=self.tempAOIh
    activeAOI.position[2]=self.tempAOIxc + self.tempAOIw
    activeAOI.position[3]=self.tempAOIyc + self.tempAOIh
    activeAOI.angle = self.tempAOIa

    if activeAOI.position[0] < 1: activeAOI.position[0] = 1
    if activeAOI.position[1] < 1: activeAOI.position[1] = 1

    if activeAOI.angle == 0:
        activeAOI.isangled = False
    else:
        activeAOI.isangled = True
        print("All fit is incorrect! It is based on the patch rotated back to horizontal w.r.t. upper left corner")
    ...
    self.AOIxc_Primary.SetValue(str(activeAOI.position[0]))
    self.AOIyc_Primary.SetValue(str(activeAOI.position[1]))
    self.AOIw_Primary.SetValue(str(self.tempAOIw))
    self.AOIh_Primary.SetValue(str(self.tempAOIh))
    self.AOIa_Primary.SetValue(str(self.tempAOIa))
