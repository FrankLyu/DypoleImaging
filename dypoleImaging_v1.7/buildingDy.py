    ...
    def dorotation(self):
        ...
        try:
            ...
            for i in range(len(self.imageData)):
                self.imageData[i] = self.rotateImage(self.imageData[i], rotangle, [self.imagePivotX, self.imagePivotY])    
        except Exception:
...
######################Read the image data when new files go into andor/images and do transform###########################
    ...
    def setRawDataFromCamera(self, imagePathList):   # called when received images from camera
        ...
        try:
            if self.isRotationNeeded is True:
                for i in range(len(self.imageData)):
                        self.imageData[i] = self.rotateImage(self.imageData[i], self.imageAngle, [self.imagePivotX, self.imagePivotY])
 ...
 #######After AOI selected, count atom number and do fitting for your AOI, show 1D fit on axe2,3############
    ...
    def Fit2DGaussian(self):
        for AAOI in self.AOIList:
            ...
            AAOI.xc2D, AAOI.yc2D, AAOI.x_width2D, AAOI.y_width2D, AAOI.modifiedrawAtomNumber, AAOI.isFit2DSuc=gaussianFit2D(AAOI.PrimaryImage,Initialparams)
            AAOI.modifiedAtomNumber = AAOI.modifiedrawAtomNumber * (self.pixelToDistance**2)/self.crossSection

    def Fit2DBoson(self):
        for AAOI in self.AOIList:
            Initialparams = [AAOI.x_width,AAOI.y_width,AAOI.x_center,AAOI.y_center,AAOI.position[0],AAOI.position[1],AAOI.isXFitSuccessful,AAOI.isYFitSuccessful]
            AAOI.xc2D, AAOI.yc2D, AAOI.x_width2D, AAOI.tfx_width2D, AAOI.tfy_width2D, AAOI.modifiedrawAtomNumber, AAOI.BECfraction, AAOI.isFit2DSuc,AAOI.x_fitted,AAOI.y_fitted,self.Zfit=BosonFit2D(AAOI.PrimaryImage,Initialparams)
            AAOI.modifiedAtomNumber = AAOI.modifiedrawAtomNumber * (self.pixelToDistance**2)/self.crossSection
...
#            print "----------Let's kill the old observer---------------------"
        ...
        if self.buttonSaveDummy.GetValue(): # if you save a dummy image
            ...
            try:
                ...
                writeImageToCacheC(atomShotCropList,
                               ...
                               darkShotCropList,
                               300 ,
                               300 ,   # 300 is the width and height of camera ID =7. Check it in mysql.
                               7,
                               lastRunID,
                               lastSequenceID)
            except:
    ...
    def dictionnaryAnalysisResults(self):
        ...
        if self.doubleAOIs:
            analysisResults["nCount2"] = int(self.primaryAOI.atomNumber)
...
#######################################
    ...
    def convertCropImagesToList(self):
        ...
        atom = self.imageData[0].astype(np.int16)[XS:XS+300, YS:YS+300].ravel().tolist()
        noatom = self.imageData[1].astype(np.int16)[XS:XS+300, YS:YS+300].ravel().tolist()
        dark = self.imageData[2][XS:XS+300, YS:YS+300].astype(np.int16).ravel().tolist()
        print(atom)
...
if __name__ == '__main__':
    app = wx.App()
    ui = ImageUI(None, title='Atom Image Analysis Dy v1.7')
    app.MainLoop()