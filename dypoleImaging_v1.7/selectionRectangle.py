...
class selectionRectangle():
    ...
    def update_patch(self):
        ...
        if not self.isangled:
            self.patch.angle = 0
            if hasattr(self, "labelText"):
                xpos = np.min(self.position[[0, 2]])
                ypos = np.min(self.position[[1, 3]])
                self.labelText.set(x = xpos + 2*self.labelTextPadding, y = ypos + 2*self.labelTextPadding)
        if self.isangled:
            self.patch.angle = self.angle
    ...
    def update_image(self, imageData):
        ...
        if not self.isangled:
            ...
            # If select primary AOI
                ...
                self.rawAtomNumber = np.sum( (self.ODImg > -10) * self.ODImg)
                print(imageData.shape[1:3])
        if self.isangled:
            pass
