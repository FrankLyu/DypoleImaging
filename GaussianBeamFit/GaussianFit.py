...
class ImageUI(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.cameraType = "TIF" # FLIR or Andor. It is the starting default variable
        self.camera = Camera(self.cameraType, self.cameraPosition) # check if I can bind that to the original button position
        self.pathWrittenImages = LOCAL_CAMERA_PATH_ANDOR # folder where all the images get written by the cameras
...
#        self.degenFitter = degenerateFitter()
    ...
    from _InitUIstuff import InitUI, startCamera_trigger, endCamera_trigger, onAtomRadioClicked, updateFittingResult, TOFcalc
...
#Show Img on the main canvas axe1, another main function of this part is AOI selection, SEPARATE coded in _AOIstuff.py#
    ...
    def setCurrentImg(self, data, hasFileSizeChanged):
        Vmax = np.max(self.imageData)
        if hasFileSizeChanged or self.currentImg is None:
            print("showing image")
            self.axes1.cla()
            self.currentImg = self.axes1.imshow(data, cmap=self.cmap, aspect='equal', vmin=-0.1, vmax=Vmax) #self.cmap defined in _InitUIstuff.py
            self.setZoomImage(data)
        else:
            print("reshowing that image")
            self.axes1.cla()
            self.currentImg = self.axes1.imshow(data, cmap=self.cmap, aspect='equal', vmin=-0.1, vmax=Vmax)
            self.setZoomImage(data)
        self.initializeAOI()
    ...
    def updateImageOnUI(self, layerNumber, hasFileSizeChanged):
        ...
        if self.isDummyImage:
            self.setCurrentImg(self.imageData, hasFileSizeChanged)
            self.currentImg.autoscale()
        else:
            if not self.imageData==0:
                if layerNumber == 4:
    ...
    def showImgValue(self, e):
        if e.xdata and e.ydata:
            x = int(e.xdata)
            y = int(e.ydata)
            if (self.imageData.size>0) and (x >= 0  and x < self.imageData.shape[1]) and (y >= 0 and y < self.imageData.shape[0]):
                self.cursorX.SetValue(str(x))
                self.cursorY.SetValue(str(y))
                if self.layer1Button.GetValue():
                    self.cursorZ.SetValue(str(int(self.imageData[y][x])))
...
if __name__ == '__main__':
    app = wx.App()
    ui = ImageUI(None, title='Gaussian Beam analysis')
    app.MainLoop()