...
def setZoomImage(self, data):
    ...
    self.axesZoom.cla()
    self.currentZoomImage = self.axesZoom.imshow(dataZoom, cmap=self.cmap, aspect='equal', vmin=-0.1, vmax=np.max(data), extent=[self.xMinZoom, self.xMaxZoom, self.yMaxZoom, self.yMinZoom])
...
def showImgValueZoom(self, e):
    if e.xdata and e.ydata:
        x = int(e.xdata)
        y = int(e.ydata)
        if (not self.imageData==0) and (x >= 0  and x < self.imageData[0].shape[1]) and (y >= 0 and y < self.imageData.shape[0]):
