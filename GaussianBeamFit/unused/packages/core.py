...
#        print "Save for snippet server"
    ...
    def startCamera(self, event):
        ...
        self.cameraThread = threading.Thread(target = self.camera.cameraDevice.run_single_camera) #, args=[i])
        self.cameraThread.start()
    
    def endCamera(self, event):
...
#                self.observer.start()
                ...
                self.monitor = Monitor(self.pathWrittenImages, self.autoRun, self.expectedFileSize, self.camera)
...
#            print "----------Let's kill the old observer---------------------"
    ...
    def dictionnaryAnalysisResults(self):
        analysisResults = {
            ...
            "xWidth" : int(self.x_width),
            "yWidth" : int(self.y_width),
            "xPos" : int(self.x_center),
            "yPos" : int(self.y_center),
            #"PSD" : (self.x_width*self.y_width)**3
            "PSD" : 0
            }
...
#######################################
    ...
    def convertImagesToList(self):
        return self.imageData[0].ravel().tolist(), self.imageData[1].ravel().tolist(), self.imageData[2].ravel().tolist()
