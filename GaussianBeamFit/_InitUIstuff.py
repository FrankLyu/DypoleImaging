...
import datetime
from config import LOCAL_PICTURE_PATH
...
def InitUI(self):
    ...
    self.FLIRCamera = wx.RadioButton(self.panel, label="TIFF", style = wx.RB_GROUP)
    self.AndorCamera = wx.RadioButton(self.panel, label="TIF")
    ...
    self.verticalCamera.Disable()
    self.horizontalCamera.Disable()
    self.TestCamera.Disable()
...
#        tempSizer = wx.StaticBoxSizer(tempStaticBox, wx.HORIZONTAL)
    ...
    self.FitResultText = wx.TextCtrl(self.panel, value = "REMEMBER to set mag,pixel and \n click Auto Read!", style=wx.TE_MULTILINE | wx.TE_READONLY, size = (420, 300))
    ...
    imageZoomBoxSizer.Add(hboxZoomInput)
    ...
    imageZoomBoxSizer.Add(hboxZoomOutput)
    vbox2.Add(imageZoomBoxSizer, border = 5)
...
def updateFittingResult(self):
    ...
    for AAOI in self.AOIList:
        ...
        self.resultstring += f"1D fit x_width: {AAOI.x_width:.1f} pixels. Note here fit is e^(-2*x^2/x_width^2), without any slope. \n"
        self.resultstring += f"1D fit x_width: {AAOI.x_width*self.pixelToDistance*1e6:.1f} um "
        self.resultstring += f". y_width: {AAOI.y_width*self.pixelToDistance*1e6:.1f} um\n"
        self.resultstring += f"2D fit width: {AAOI.x_width2D*self.pixelToDistance*1e6:.1f} um, {AAOI.y_width2D*self.pixelToDistance*1e6:.1f} um\n\n" 
