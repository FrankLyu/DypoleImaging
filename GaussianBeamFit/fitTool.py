...
def single_G(x, A, center, sigma, offset):
    return A*np.exp(-1*(2*(x-center)**2)/(sigma**2)) + offset 

def single_G_plus_slope(x, A, center, sigma, offset, slope):
    return A*np.exp(-1*(2*(x-center)**2)/(sigma**2)) + offset + 0 * x

def gaussian_2d(xy, A, x0, y0, sigma_x, sigma_y, offset):
    x,y=xy
    exponent = -((x - x0)**2 / (0.5 * sigma_x**2) + (y - y0)**2 / (0.5 * sigma_y**2))
...
def gaussianFit2D(data,Initialparams):
    ...
    Guesstop = np.max(data)
    X, Y = np.meshgrid(x, y)
    Z = data  # Your 2D data
    # Initial guess for the parameters
    # initial guess ODmax = 1
    initial_guess = (Guesstop, Guessx_center, Guessy_center, Guessx_width, Guessy_width, 0.0)
    # Perform the fit
...
###############################################################################
    ...
    while (num_trial <= max_num_trial and isFitSuccessful is False):
        try:
            ...
            x_slope = 0  # Modified on 03/04/2024 to eliminate possible slope.
...
def bosonDistribution(xy, x0, y0, a, amplitudeC, offset, amplitudeT, Ca, Cb):
    """BosonParams = ?"""
    x,y=xy
    thermalPart = amplitudeT * np.exp(- (x - x0) **2/a**2 - (y - y0)**2/a**2)
    condensatePart = amplitudeC *  np.maximum( (1 - Ca * (x - x0)**2 - Cb * (y - y0)**2), 0)
    dist = thermalPart + condensatePart + offset
    return dist.ravel()
    
def BosonFit2D(data,Initialparams):
    Guessx_width = Initialparams[0]
    Guessx_center = Initialparams[1]
    Guessy_center = Initialparams[2]
    x_start = Initialparams[3]
    y_start = Initialparams[4]
    IsXSuc = Initialparams[5]
    IsYSuc = Initialparams[6]
    isFitSuccessful = False
    if not IsXSuc:
        Guessx_center = x_start + data.shape[1]//2
        Guessx_width = data.shape[1]//4
    ...
    # Initial guess for the parameters
    initial_guess = (Guessx_center, Guessy_center, Guessx_width,1, 0.0, 0.5, Guessx_width, Guessy_width)
    # Perform the fit
    if data.shape[0]*data.shape[1]>500*500:
        ...
        try:
            params, covariance = curve_fit(BosonFit2D, (X, Y), Z.ravel(), p0=initial_guess)
            # Unpack the parameters
            xc, yc, sigma_x, Ac, offset, At, TF_x, TF_y = params
            isFitSuccessful = True
            modifiedrawAtomNumber = np.sum(data)-offset*np.size(data)
            def CondensateAmp(x, y, Ac, TF_x, TF_y, xc, yc):
                return Ac * np.maximum( (1 - TF_x * (x - xc)**2 - TF_y * (y - yc)**2), 0)
            CondensateSum = 0
            for xind in x:
                for yind in y:
                    CondensateSum = CondensateSum + CondensateSum(xind,yind,Ac,TF_x,TF_y,xc,yc)
            print("jl")
            print(CondensateSum) 
        except Exception:
            xc, yc, sigma_x, sigma_y, Ac, offset, At, TF_x, TF_y= 0,0,0,0,0,0,0,0,0
