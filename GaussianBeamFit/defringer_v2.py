...
#        S = np.diag(s)
#        recovered =  np.dot(U, np.dot(S, V.T))
#        recovered2 = np.dot(U[:, :numOfDim], np.dot(S[:numOfDim,:numOfDim], V[:,:numOfDim].T))
#        
#        print(V[:, :numOfDim].shape)
#        ## origianl means
...
#        print absImgFileName
#        print "--------------------------------------------------------------"        
#        print noAtomFileNameList
...
#            signal = temp[roiIndex[1]:roiIndex[3],roiIndex[0]:roiIndex[2]]
            temp[self.yTop-3:self.yBottom+4, self.xLeft-3:self.xRight+4] = 0
            coeff = np.dot(temp.flatten(), noAtomList.T)
        #    principal = tester.mean + np.dot(V, coeff) 
            ref = np.dot(noAtomList.T, coeff)
        #    print principal.shape
#            ref = principal.reshape(absImage.shape)
